...
@login_required
def list_shipments():
    """Devuelve la lista de todos los enví­os."""
...
@login_required
def get_shipment(id):
    """Devuelve un enví­o especí­fico."""
    envio = Shipment.query.get(id)
    if not envio:
        return jsonify({'success': False, 'error': 'Enví­o no encontrado'}), 404
...
@login_required
def create_shipment():
    """Crea un nuevo enví­o para un pedido."""
    ...
    if not pedido_id or not direccion_envio:
        return jsonify({'success': False, 'error': 'Pedido y dirección de enví­o son obligatorios'}), 400
    ...
    if numero_guia and Shipment.query.filter_by(numero_guia=numero_guia).first():
        return jsonify({'success': False, 'error': 'Ese níºmero de guí­a ya está registrado'}), 409
    ...
    return jsonify({
        'success': True,
        'message': f'Enví­o #{envio.id} creado exitosamente',
        'envio': envio.to_dict()
    }), 201
...
def update_shipment(id):
    ...
    if not direccion_envio:
        return jsonify({'success': False, 'error': 'La dirección de enví­o es obligatoria'}), 400

    existente = Shipment.query.filter_by(numero_guia=numero_guia).first() if numero_guia else None
    if existente and existente.id != id:
        return jsonify({'success': False, 'error': 'Ese níºmero de guí­a ya está en uso'}), 409
    ...
    if data.get('fecha_envio'):
        try:
            ...
            return jsonify({'success': False, 'error': 'Formato de fecha de enví­o inválido'}), 400
    ...
    return jsonify({
        'success': True,
        'message': 'Enví­o actualizado exitosamente',
        'envio': envio.to_dict()
    })
...
def delete_shipment(id):
    ...
    return jsonify({
        'success': True,
        'message': f'Enví­o #{id} eliminado correctamente'
    })
